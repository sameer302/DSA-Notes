...
- When more than one key maps to the same index it is called as collision. The most common method to handle collision (which is also used in unordered_map in c++) is separate chaining. In this each array element stores a linked list (or similar container).
...
- Implementations ensure that Number of buckets ~ constat x Number of elements, hence space complexity always stays O(n).
